...
agent 造大于单个 session 的东西时，真正重要的是三件事：

**1. 绘制解空间的地图。** 哪里已经解开——验证过，谁也不能悄悄弄坏；哪里还开着；
哪条路试过、失败了、为什么——全留着，下一个 agent 不用再掉同一个坑。这张地图就是
仓库本身：`contracts/` 是节点，判决标出已解开的区域，失败的提交和废止记录标出探索
过的死路，`node tools/graph.mjs --format json` 把整张图打出来——每个节点的尝试史、
放弃原因、每个维度的底线——交给任何工具渲染。

**2. 拆解跟着任务走，从不预先定死。** 开工时你不知道正确的拆法是什么、哪些维度需要
优化。所以按今天的理解拆；做的过程中学到更好的拆法，就废止重拆，历史全留着。维度
也是同样被发现的：一个变慢的页面、一段没人读得懂的模块、一句说了谎的文档。

**3. 每个维度一块地基，处处是棘轮。** "能跑"只是一个维度，它的地基是验收门——
一旦变绿，就再不许悄悄变红。任务暴露出的其它维度，在咬人的那一刻同样处理：量出
今天的水平，冻结成一条 law（`laws/<id>.json`，验证器每一遍都跑的检查），从此这块
地盘不会无声丢失。进步 = 关闭开放契约 + 有意收紧底线，永不后退。

**诚实的边界**：一个 session 装得下的任务，直接在那个 session 里做更便宜。这套
东西在工作比工人活得久时才回本——session 会结束，地图留下来。

规模上的先例是 [Prove2Me](https://arxiv.org/abs/2608.28433)：它让一群 Claude agent 在 11 天内[完成了费马大定理的 Lean 形式化](https://www.anthropic.com/research/formalizing-fermats-last-theorem)：三万条定理无人逐条审阅，信任完全落在"检查器 + 不可变的、经人工审计的陈述"上。它的作者先试过两条显然的路——多 agent 共同编辑文件（互相干扰、无法分割）和 git PR 流程（人工审并成为瓶颈）——都失败了才发明这套协议。而这两个失败恰好就是今天多 agent 写软件的现状。
...
- **判决只裁契约，不归功劳**：门按契约对工作树运行，同一契约多份提交就绪时，
  决定谁显示 ACCEPTED 的是目录顺序而不是因果——一份把别人产物写进 files 的提交
  可以冒领。把判决绑定到提交声明的产物是待做项。
- **门可变，契约不可变**：CI 保护 `contracts/` 与废止日志，尚无机制保护
  `acceptance/`——后来的 commit 可以在不触发任何检查的情况下改弱一个门。计划中
  的修法：CI 强制"门必须早于该契约的第一份提交"。
- **Done 的根与开放 backlog 不能共存**：根门断言 frontier 为空，所以发布任何新
  的开放契约都会重开 root、令 main 变红，直到新契约关闭。这正是 prove2me 的
  mission 语义（完成即无开放项）——新工作要么当轮关闭，要么开新的根。
- **重复尝试要真金白银**：首个通过者胜意味着同一契约可能付 N 次钱。Race 001 丢弃了三分之二的实现——在那里值得（被丢弃的那两份暴露了缺陷和门的错误），但这是逐契约的选择，不是免费午餐。

## 一个量化的例子

`examples/ranked-search/` 是一个排序全文搜索 CLI，分三轮建成，**每轮都是一个
对前情零记忆的全新 agent**，全程在协议下：先建成，再注入中途需求变更（精确短语
查询——三份契约被废止并发布后继、依赖重指），最后一轮只许优化不许倒退。每轮由
赛前注册的隐藏评审打分；同一任务同时由不用协议的普通 agent session 平行完成，
同模型、同 prompt。
...
守住的地盘一寸未丢（任何轮次任何指标零倒退），底线带着收据移动（体积 law
0.32 → 0.38 为位置索引付费，→ 0.26 编码优化后收紧），全程 token 是普通臂的
1.15 倍——完整数字（包括普通臂更好的地方）见
[bench/002-ranked-search/results.md](bench/002-ranked-search/results.md)。
