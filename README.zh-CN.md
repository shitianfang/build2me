...
一个上下文充足的 agent session 本身已经是合格的工程师，它做不到的是**长跑**：
上下文会满、session 会死，每次重启都丢掉计划、进度和每个决定背后的理由。当任务
规模超过任何单个 session——按天计的墙钟、多个 agent、无人盯守——瓶颈就不再是
智能，而是基础设施：session 都不在了，意图放在哪里？没人审查时，"完成"由谁说了
算？下一个 agent 不经交接怎么知道该干什么？

build2me 就是这层基础设施——**长程自主运行 agent 的基础支撑**。它的每个机制都是
把其中一个问题回答成文件格式或推导规则：

| 长程失效 | 协议的回答 |
|---|---|
| 意图随 session 消亡 | `contracts/`——任务写成不可变、机器可判的陈述，比任何作者都活得久 |
| "完成"靠 agent 自报 | 验证器——状态在读取时从门导出，从不存储、从不听声明 |
| 新 agent 需要人工交接 | frontier——冷启动 agent 读一眼就知道什么开放、什么可做；不需要交接文档 |
| 并行 agent 互相干扰 | 不可变 + 乐观并发——什么都不许改，就什么都不用争 |
| 需求中途变化 | 废止级联——历史从不重写，下游机械地重新打开 |
| 经验随 session 蒸发 | 提交与失败尝试永久可搜索；laws 把赢下来的经验冻成强制规则 |

**它不为了什么**：一个 session 的上下文装得下的任务，直接在那个 session 里做更
便宜。契约、门、验证的成本买的是持久性与信任——只有当工作比工人活得久，才回本。
